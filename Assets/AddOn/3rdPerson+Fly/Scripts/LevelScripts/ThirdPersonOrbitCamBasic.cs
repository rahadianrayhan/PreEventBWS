...
public class ThirdPersonOrbitCamBasic : MonoBehaviour
{
	...
	public string XAxis = "Cam_h";									// The default horizontal axis input name.
	public string YAxis = "Cam_v";									// The default vertical axis input name.
	...
	public float SetH { set => angleH = value; }
	public float SetV { set => angleV = value; }
	...
	void Update()
	{
		...
		if (!InputControl.PauseState)
		{
			...
			// Joystick:
			angleH += Mathf.Clamp(AxisInput.GetAxis(XAxis), -1, 1) * 60 * horizontalAimingSpeed * Time.deltaTime;
			angleV += Mathf.Clamp(AxisInput.GetAxis(YAxis), -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
		}
